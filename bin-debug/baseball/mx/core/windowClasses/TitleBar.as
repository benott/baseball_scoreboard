package mx.core.windowClasses
{
    ...
    public class TitleBar extends UIComponent
    {
        ...
        protected function placeTitle(param1:String, param2:Number, param3:Number, param4:String) : void
        {
            ...
            if (param4 == "left")
            {
                _loc_5 = Math.max(closeButton.x + closeButton.measuredWidth, minimizeButton.x + minimizeButton.measuredWidth, maximizeButton.x + maximizeButton.measuredWidth);
            }
            else
            {
                _loc_5 = Math.min(closeButton.x, minimizeButton.x, maximizeButton.x);
            }
            if (param1 == "left")
            {
                if (param4 == "left")
                {
                    titleTextField.setActualSize(width - param3 - 2 - _loc_5, measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING);
                    titleTextField.move(_loc_5, (height - (measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING)) / 2);
                    titleTextField.truncateToFit();
                }
                else
                {
                    titleTextField.setActualSize(Math.max(0, _loc_5) - 2, measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING);
            ...
            else
            {
                ...
                _loc_8 = titleTextField.getLineMetrics(0).width;
                titleTextField.truncateToFit();
                if (param4 == "left")
                {
                    titleTextField.setActualSize(width - param3 - _loc_5 - 2, measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING);
                    _loc_7 = Math.max(_loc_5, (width - param2 - param3 - _loc_8) / 2);
                    titleTextField.move(_loc_7, (height - (measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING)) / 2);
                }
                else
                {
                    titleTextField.setActualSize(width - param2 - param3 - (width - _loc_5) - 2, measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING);
                    _loc_7 = Math.max(0, Math.min(_loc_5 - _loc_8 - 2, (width - param2 - param3 - _loc_8) / 2));
                    titleTextField.move(_loc_7, (height - (measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING)) / 2);
                }
            }
            return;
