package com.keaura.controls
{
	...
	public class ScrollingText extends Label
	{
		...
		[Inspectable(type="String",enumeration="leftToRight,rightToLeft,topToBottom,bottomToTop,static,blink")]
		/**
	    ...
	    private function get isHTML():Boolean
    	{
    		...
    		//if ( htmlText == null || htmlText == "" ) return false;
    		//else return true; 
    	}
		...
		override protected function commitProperties() : void
		{
            ...
            if (isHTML) {
            	htmlText = text;
                cache[0].htmlText = htmlText;
                cache[0].selectable = false;
				cache[1].htmlText = htmlText;
                cache[1].selectable = false;
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			{
				case RIGHT_TO_LEFT:
					label1.move(unscaledWidth,ypos);
					label0.move(Math.max((unscaledWidth+20),label1.width+20),ypos);
					break;
				case LEFT_TO_RIGHT:
					label0.move(0-label0.width,ypos);
					label1.move(0-label1.width-20,ypos);
					break;
				case BOTTOM_TO_TOP:
		...
		private function moveText( event:Event ) : void
		{
			...
			if( direction == RIGHT_TO_LEFT ) {
				...
				if( cache[currentIndex].x+cache[currentIndex].width <= 0 ) {
					cache[currentIndex].x = Math.max(cache[currentIndex].parent.width,cache[secondIndex].width+20);
			...
			// BLINK 
				...
				if(messageState >= (20 / messageSpeed))
				{
					if((cache[0] as Label).visible == true)
						(cache[0] as Label).visible = false;
					else
						(cache[0] as Label).visible = true;
					messageState = 0;
				}
				else
					messageState++;
